...
import { UserModel, TutorModel, StudentRequestModel, PlatformReviewModel, ChatSessionModel } from './models.js';
...
const MOCK_TUTORS = [
  {
    ...
    city: 'New York',
    reviewsList: [
      { id: 'r1', studentName: 'Emily R.', rating: 5, comment: 'Sarah is incredible! She helped me finally understand Calculus. Highly recommend.', date: '2023-11-15' },
      { id: 'r2', studentName: 'Jason M.', rating: 5, comment: 'Very patient and explains things clearly. My grades improved significantly.', date: '2023-10-02' },
      { id: 'r3', studentName: 'Lisa K.', rating: 4, comment: 'Great tutor, but sometimes hard to schedule due to high demand.', date: '2023-09-20' }
    ]
  },
  {
    id: 'T-002',
    ...
    city: 'San Francisco',
    reviewsList: [
      { id: 'r1', studentName: 'Michael B.', rating: 5, comment: 'David helped me ace my history paper. His feedback on my essay was invaluable.', date: '2023-12-01' },
      { id: 'r2', studentName: 'Sarah L.', rating: 4, comment: 'Good knowledge of literature, very helpful for my AP English class.', date: '2023-11-10' }
    ]
  },
  {
    id: 'T-003',
    ...
    city: 'Chicago',
    reviewsList: [
      { id: 'r1', studentName: 'Tom H.', rating: 5, comment: 'Fun and engaging lessons! I feel much more confident speaking Spanish now.', date: '2023-10-25' }
    ]
  },
  {
    id: 'T-004',
    ...
    city: 'Austin',
    reviewsList: [
      { id: 'r1', studentName: 'Alex P.', rating: 5, comment: 'Michael knows his stuff. He helped me prepare for my coding interviews perfectly.', date: '2024-01-10' },
      { id: 'r2', studentName: 'Jessica W.', rating: 5, comment: 'Best coding tutor I have found. Explains complex algorithms simply.', date: '2023-12-15' }
    ]
  },
  {
    id: 'T-005',
    ...
    city: 'New York',
    reviewsList: [
      { id: 'r1', studentName: 'Parent of Timmy', rating: 5, comment: 'My son loves his piano lessons with Jessica. She is so patient.', date: '2023-11-05' }
    ]
  },
  {
    id: 'T-006',
    ...
    city: 'Boston',
    reviewsList: [
      { id: 'r1', studentName: 'Chris D.', rating: 5, comment: 'Robert is a legend. He knows exactly what is on the AP exams.', date: '2023-05-10' },
      { id: 'r2', studentName: 'Ashley T.', rating: 4, comment: 'Very detailed explanations, sometimes goes a bit fast but knows his chemistry.', date: '2023-04-22' }
    ]
  },
  {
    id: 'T-007',
...
const MOCK_REQUESTS = [
  ...
  },
  {
    ...
    status: 'matched',
    assignedTutorId: 'T-005' // Seed as matched to Jessica Lee (Piano)
  },
  {
    id: 'req3',
    studentId: 'S-003',
...
const seedDB = async () => {
  ...
  try {
    ...
    await PlatformReviewModel.deleteMany({});
    ...
    const adminUser = {
      ...
      name: 'Admin',
      email: 'admin@findateacher0143.com',
      password: 'FindATeacher@Admin#2026',
    ...
    console.log(`Seeded ${MOCK_USERS.length} students into Users collection.`);

    // Seed Tutors as both User profiles (for authentication) and Tutor profiles (for detail lists)
    const tutorUsers = MOCK_TUTORS.map(t => ({
      id: t.id,
      name: t.name,
      email: t.email || `${t.id.toLowerCase()}@example.com`,
      password: 'password123',
      avatar: t.avatar,
      role: 'tutor',
      phone: t.phone,
    ...
    await UserModel.insertMany(tutorUsers);
    console.log(`Seeded ${tutorUsers.length} tutor credentials into Users collection.`);

    await TutorModel.insertMany(MOCK_TUTORS);
    console.log(`Seeded ${MOCK_TUTORS.length} tutor profiles into Tutors collection.`);

    // Seed Student Requests
    await StudentRequestModel.insertMany(MOCK_REQUESTS);
    console.log(`Seeded ${MOCK_REQUESTS.length} requests.`);

    // Seed Platform Reviews
    await PlatformReviewModel.insertMany(INITIAL_REVIEWS);
    console.log(`Seeded ${INITIAL_REVIEWS.length} platform reviews.`);

    // Seed initial demo chat sessions if needed
    // We will create one initial chat session between Alex (student) and Sarah (tutor) since they are matched/linked,
    // plus a session between Noah Kim (S-005) and Admin for logs, or similar. Let's create an initial chat.
    const demoSession = {
      id: 'chat-st-demo',
      participantIds: ['S-001', 'T-001'],
      messages: [
        {
          id: 'msg-seed-1',
          senderId: 'admin-system-user',
          text: 'Admin linked Student Alex Mitchell with Tutor Sarah Jenkins. Chat is now enabled.',
          timestamp: new Date(Date.now() - 3600000 * 2)
        },
        {
          id: 'msg-seed-2',
          senderId: 'S-001',
          text: 'Hi Sarah! I saw your profile and requested physics tutoring. Can we schedule a session?',
          timestamp: new Date(Date.now() - 3600000 * 1.9)
        },
        {
          id: 'msg-seed-3',
          senderId: 'T-001',
          text: 'Hi Alex! Yes, absolutely. I am available during weekdays after 4 PM. Does that work for you?',
          timestamp: new Date(Date.now() - 3600000 * 1.5)
        }
      ],
      lastMessagePreview: 'Hi Alex! Yes, absolutely. I am available...',
      updatedAt: new Date(Date.now() - 3600000 * 1.5),
      unreadCount: 0
    };
    await ChatSessionModel.create(demoSession);
    console.log("Demo chat session seeded.");
