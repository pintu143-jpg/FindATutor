...
const StudentRequestWizard: React.FC<StudentRequestWizardProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="space-y-4 animate-in slide-in-from-right duration-300">
            ...
            <div className="relative">
              ...
              <input
                ...
                autoFocus
                onKeyDown={(e) => e.key === 'Enter' && data.level && handleSubmit(data)}
              />
            </div>
            <button
              onClick={nextStep}
      ...
      case 7:
        return (
          <div className="space-y-4 animate-in slide-in-from-right duration-300">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">What is your budget? (₹/hr)</h3>
            <div className="relative">
              ...
              <input
                ...
                autoFocus
                onKeyDown={(e) => e.key === 'Enter' && data.budget && handleSubmit(data)}
              />
            </div>
            <button
  ...
  const handleSubmit = async (finalData: WizardData) => {
    try {
      ...
      // Fallback to local completion if API fails, or show error
      // For now, we'll still call onComplete to not block the UI flow, 
      // but in a real app you might want to show an error toast.
      onComplete(finalData);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">Step {step} of 7</span>
          </div>
