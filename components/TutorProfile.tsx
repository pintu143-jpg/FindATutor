...
const TutorProfile: React.FC<TutorProfileProps> = ({ tutor, onBack, onStartChat, onBookLesson, onAddReview, currentUser, showToast }) => {
  ...
  const handleBookClick = () => {
    if (!currentUser) {
        if (showToast) {
            showToast('Authentication Required', 'Please log in to book a lesson.', 'info');
        } else {
             ...
             <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 sticky top-32 transition-all">
                <div className="flex justify-between items-end mb-6 pb-6 border-b border-slate-100 dark:border-slate-700">
                   <div>
                     ...
                     <div className="flex items-center gap-1.5 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2.5 py-1 rounded-full text-xs font-semibold">
