...
import { Send, User as UserIcon, MoreVertical, MessageCircle, ArrowLeft, Users, Shield, Search } from 'lucide-react';
...
const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  const getLastMessagePreview = (session: ChatSession) => {
    const visibleMessages = session.messages.filter(msg => {
      if (msg.id.startsWith('msg-log-')) {
        return currentUser.isAdmin || session.participantIds.includes(currentUser.id);
      }
      return true;
    });
    ...
    return visibleMessages[visibleMessages.length - 1].text;
  };
      ...
      <div className={`w-full md:w-80 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col ${activeSessionId ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <button 
              onClick={onBack} 
              className="p-2 -ml-2 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"
              title="Back to Home"
            >
          ...
          <div className="relative">
             ...
             <input 
               ...
               className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-700 border-none rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white text-xs placeholder-slate-400"
             />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {(() => {
            const filteredSessions = sessions.filter(session => {
              ...
              const matchesRole = info.role?.toLowerCase().includes(query) || false;
              const matchesSubtext = info.subtext?.toLowerCase().includes(query) || false;
              const matchesLastMsg = getLastMessagePreview(session).toLowerCase().includes(query);
              const matchesId = session.participantIds.some(id => id.toLowerCase().includes(query));
              
              return matchesName || matchesRole || matchesSubtext || matchesLastMsg || matchesId;
            });
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-1">
                      <h4 className={`font-semibold truncate text-sm ${isActive ? 'text-indigo-900 dark:text-indigo-300' : 'text-slate-900 dark:text-white'} ${hasUnread ? 'font-bold' : ''}`}>
                          {info.name}
                      </h4>
                    ...
                    <div className="flex items-center gap-1 justify-between">
                        <div className="flex items-center gap-1 min-w-0 flex-1">
                            {info.isMonitoring && <Shield className="w-3 h-3 text-slate-400 dark:text-slate-500 flex-shrink-0" />}
                            <p className={`text-xs truncate ${isActive ? 'text-indigo-700 dark:text-indigo-300 font-medium' : 'text-slate-500 dark:text-slate-400'} ${hasUnread ? 'font-bold text-slate-900 dark:text-white' : ''}`}>
                            {getLastMessagePreview(session)}
                            </p>
                        </div>
                        {hasUnread && (
                            <span className="ml-2 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] h-4 flex items-center justify-center text-center">
                                {session.unreadCount}
                            </span>
                        )}
                    </div>
        ...
        <div className={`flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-900 ${!activeSessionId ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {activeSession.messages
              .filter(msg => {
                if (msg.id.startsWith('msg-log-')) {
                  return currentUser.isAdmin || activeSession.participantIds.includes(currentUser.id);
                }
                return true;
              })
              .map((msg) => {
          ...
          {/* Input */}
          {getParticipantInfo(activeSession).isMonitoring ? (
            <div className="bg-slate-100 dark:bg-slate-800/80 p-4 border-t border-slate-200 dark:border-slate-700 text-center text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-center justify-center gap-2">
              <Shield className="w-4 h-4 text-indigo-500 animate-pulse" />
              <span>You are monitoring this conversation. Only participants can send messages.</span>
            </div>
          ) : (
