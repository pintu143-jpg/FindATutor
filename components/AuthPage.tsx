...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin, onNavigateHome, existingUsers, onRegister }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isAdmin = email.toLowerCase() === 'admin@findateacher0143.com';

    if (activeTab === 'login') {
        // LOGIN LOGIC
        try {
            const loggedInUser = await apiService.auth.login(email, password);
            setIsLoading(false);
        ...
        try {
            const registeredUser = await apiService.auth.register(newUserBody);
  ...
  const fillDemoCredentials = (type: 'student' | 'tutor' | 'admin') => {
    ...
    if (type === 'student') {
      ...
      setEmail('admin@findateacher0143.com');
      setPassword('FindATeacher@Admin#2026');
    }
  };
